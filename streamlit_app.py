...
if st.button("🔌 TRANSMITIR SEÑAL Y COMPILAR EN S_FLOW", use_container_width=True):
    ...
    # SÍNTESIS DIGITAL NATIVA COMPLETA: BEAT REAL DE TRAP CHILENO (808 + SYNT)
    sr = 22050
    duracion_beat = 4.0  
    t_vector = np.linspace(0, duration_beat if 'duration_beat' in locals() else duracion_beat, int(sr * duracion_beat), endpoint=False)
