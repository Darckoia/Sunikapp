...
class MotorStudioPro:
    def __init__(self, ruta_audio_entrada):
        ...
        Implementa un bypass seguro por si el archivo requiere decodificación avanzada.
        """
        self.ruta_entrada = ruta_audio_entrada
        try:
            ...
            # Bypass de contingencia: Genera una señal sinusoidal limpia de referencia
            # si el codec del celular sube un MP3 camuflado o un WAV corrupto
            self.sr = 44100
            t = np.linspace(0, 5, self.sr * 5, endpoint=False)
            self.audio_norm = np.sin(2 * np.pi * 440 * t) * 0.5
            self.error_lectura = True

    def aplicar_pultec_eq(self, lowcut=60, highcut=16000, orden_filtro=4):
        """ Emulación de Ecualizador Pultec: Limpia ruidos sordos graves y añade brillo """
        nyquist = 0.5 * self.sr
    ...
    def reverb_estudio_3d(self, datos_audio, retraso_ms=45, atenuacion=0.35):
        """ Genera espacialidad tridimensional emulando el tamaño de sala """
    ...
    def procesar_cadena_master(self, activar_eq, activar_ssl, nivel_reverb, ruta_salida="audio_cache/master_real.wav"):
        ...
        if nivel_reverb > 0:
            factor_atenuacion = (nivel_reverb / 100.0) * 0.4
